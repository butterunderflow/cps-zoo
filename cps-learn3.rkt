...
(define (eval3 ex env cont)
  ...
  (match ex
    ...
    [`(lambda ,sym ,body)
     (cont (lambda (x k)
             (eval3 body (cons (cons sym x) env) k)))]
                        ...
                        (let ((new-cont0
                               (lambda (v0)
                                 (let ((new-cont1
                                   ...
                                   (eval3 arg1 env new-cont1)))))
                          (eval3 arg0 env new-cont0))]
    [`(if ,ex0 ,ex1 ,ex2)
     (let ((new-cont (lambda (v)
                       (if v
                           (eval3 ex1 env cont)
                           (eval3 ex2 env cont)))))
     (eval3 ex0 env new-cont))]
    [`(begin ,@exs1)
     (eval-list eval3 exs1 env cont)]
    [`(call/cc ,ex)
     ...
     (let ((new-cont0
            (lambda (v0)
              (let ((new-cont1
                     (lambda (v1)
                       (v0 v1 cont))))
                (eval3 arg env new-cont1)))))
       (eval3 op env new-cont0))]))
...
(define (eval-top ex)
  (eval3 ex default-env init-cont))
